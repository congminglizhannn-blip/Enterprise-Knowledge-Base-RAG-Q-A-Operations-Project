...
- 引用面板按 `document_id` 合并，同一文章展示本次命中的所有片段，并按 `chunk_id` 去重；不能把同名但不同 ID 的文章合并，也不能把整篇文章未命中的 chunk 作为本次回答的引用。
...
- 历史列表使用 `/api/qa-history` 返回的 `round_count`；兼容会话接口仍返回 `qa_round_count`。轮数与两条消息、更新时间同事务保存，一问一答记 1 轮，使用 SQL 原子增量，不能使用浏览器缓存消息长度。
- 历史审计统一走 DataScope：超级管理员全量、部门管理员同组织本部门树、普通用户本人。列表、详情和筛选选项必须统一校验；审计读取权限不得用于代他人续聊或删除他人会话。
- 会话组织/部门必须来自提问用户快照，不得取知识库归属；调岗或换库后新建会话，保留旧归属。部门层级修改禁止循环及跨组织父级。
...
- 登录页不预填账号密码；角色选择必须传递到登录 API，在创建 session 和签发 token 前与数据库角色比较，不匹配即拒绝，不能先登录成功再由前端判断。
- 覆盖三种角色匹配及交叉不匹配、缺失/非法角色、错误密码和停用账号；失败不应生成 cookie/session/token，权限始终来自数据库。

## 知识库启停验收经验（代码问题）

- 数据库迁移遗漏属于配置问题：新增模型字段后，先确认 Alembic revision 与数据库实际列，再应用迁移；不能把模型查询 500 当成前端异常处理问题。聊天页知识库与历史应独立请求，单个接口失败应显示对应错误，不阻断另一列表。

- 知识库禁用必须持久化，保留文档、向量和历史；管理列表通过 `include_disabled=true` 获取可管理的禁用项，业务列表默认只显示启用项。
- 启停权限与知识库管理权限一致；业务访问和文档写入统一校验状态，向量与关键词检索同时过滤 `is_active`，不能只禁用前端按钮。
- 新增状态字段必须先执行 Alembic migration，再启动新版后端；禁用后验证数据计数不变，重新启用后验证原文档恢复访问。
...
## 页面切换与认证加载经验（代码与开发环境问题）

- 根 AuthProvider 只在首次挂载恢复认证；业务路由共享 `(business)/layout.tsx` 的 AuthGate 与 AppShell，禁止每个页面重复创建公共布局。路由 loading 只占内容区域。
- 首次认证、已登录后台验证和业务数据加载分别处理；后台网络失败保留当前页面并提示，确认 401 才清空认证。认证超时必须取消请求，登录/退出后禁止旧认证响应覆盖新状态。
- 业务只读短缓存不落盘，写操作、显式刷新、身份切换及 401 必须失效；不缓存认证、历史审计或带取消信号的请求。
- Next 开发输出 `.next` 与生产输出 `.next-production` 分离，类型检查也分离。开发环境首次路由编译不能作为生产导航性能结论。

- 聊天侧栏与审计历史语义不同：`/chat` 使用 `/api/sessions?scope=mine`，所有角色只列本人；`/api/qa-history` 保留角色审计范围。不得把管理员审计可见范围当成个人可续聊列表。
- `/chat` 的新增对话先创建本人空会话，用户在该会话内发送首个问题后，默认标题 `新会话` 更新为首个问题标题；不能另外生成一个以问题命名的新会话，留下空的 `新会话`。
- 文档列表跨知识库合并后必须按文档 `created_at desc, id desc` 全局排序；不能按知识库请求顺序直接拼接，否则新上传的待解析文档可能出现在列表中部。
